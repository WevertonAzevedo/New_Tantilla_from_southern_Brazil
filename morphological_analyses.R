...
# To configure your dataset, please follow these procedures (they are the same for all other scripts):
...
# 4 - Name all your morphological variables with the prefixes: "cont_" for continuous variables, "cat_" for categorical variables, rat_ for rates between variables, and "disc_" for discrete variables;
...
# 6 - Have a column named "ID" with a specific ID label for each row.

# X - Your table need to be in csv format.

# OBS01 - I like loops... I know that it is not too "R style" and can perform badly when compared... feel free to apply "apply" functions when they can be applied...
...
# needed for PCA/LDA plot
library(ggbiplot)

#install.packages("ggplot2")
# needed for multivatiated plots
library(ggplot2)
...
# create a dataset without "sample to test"

x<-x[x[,grep("OTUs",colnames(x))[1]]!="to_test",]

#######################

# read OTUs and cat variables as factor
for(i in 1:length(grep("OTUs",colnames(x)))){
	x[,grep("OTUs",colnames(x))[i]]<-as.factor(x[,grep("OTUs",colnames(x))[i]])
}

for(i in 1:length(grep("Type",colnames(x)))){
	x[,grep("Type",colnames(x))[i]]<-as.factor(x[,grep("Type",colnames(x))[i]])
}
...
dir.create("./00_preliminary_plots",showWarnings = FALSE)
...
# Preliminary plots to check correlation between cont variables
bodylength<-x[,grep("SVL",colnames(x))]-x[,grep("HL",colnames(x))]

for(i in seq(x_cont[1,])){
	pdf(paste("./00_preliminary_plots/plot_",colnames(x_cont)[i],".pdf",sep=""), family="ArialMT", width=9, height=12)
		plot(bodylength,x_cont[,i],ylab=colnames(x_cont)[i])
	dev.off()
}
...
x<-cbind(x,x_ratio)
x_all<-cbind(x_cont,x_disc,x_cat,x_ratio)
x_multi<-cbind(x_disc,x_cat,x_ratio)
...
sink("./01_tables/Removed_Characters.txt")
	print(to_remove)
sink()

# cleaning dataset from bad variables
x_cln0<-list()
for(k in seq(Hlabel)){
	x_cln0[[Hlabel[k]]]<-x[,!colnames(x)%in%to_remove[[k]]]
}

x_all_cln0<-list()
for(k in seq(Hlabel)){
	x_all_cln0[[Hlabel[k]]]<-x_all[,!colnames(x_all)%in%to_remove[[k]]]
}

x_multi_cln0<-list()
for(k in seq(Hlabel)){
	x_multi_cln0[[Hlabel[k]]]<-x_multi[,!colnames(x_multi)%in%to_remove[[k]]]
}
...
for(k in seq(Hlabel)){
	...
	#loop for each variable
	for(i in 1:ncol(x_all_cln0[[k]])){
		#open pdf file
		pdf(paste("./04_boxplots/02_all_variables-",Hlabel[k],"-check/boxplot_",colnames(x_all_cln0[[k]])[i],".pdf",sep=""),family="ArialMT", width=length(OTUs[[k]]), height=12)
			#plot boxplot
			par(mar = c(7, 4, 4, 2) + 0.1)
			boxplot(x_all_cln0[[k]][,i] ~ x[,Hindex[k]],main=colnames(x_all_cln0[[k]])[i],outline=F,ylim=c(min(na.omit(x_all_cln0[[k]][,i])),max(na.omit(x_all_cln0[[k]][,i]))),boxwex = 0.8,xaxt = "n", xlab= Hlabel[k], ylab ="")
	# 		axis(1, labels = FALSE)
			...
			for(j in 1:length(levels(x[,Hindex[k]]))){
				if(length(na.omit(x_all_cln0[[k]][,i][x[,Hindex[k]]==levels(x[,Hindex[k]])[j]]))==0){
					#plot N for variable
					text(j,mean(na.omit(x_all_cln0[[k]][,i])), labels=paste("N = ",length(na.omit(x_all_cln0[[k]][,i][x[,Hindex[k]]==levels(x[,Hindex[k]])[j]])),sep=""),cex=0.8,pos=3)
				}
				else{
					#get histogram data
					b<-hist(x_all_cln0[[k]][,i][x[,Hindex[k]]==levels(x[,Hindex[k]])[j]],breaks=length(x_all_cln0[[k]][,i][x[,Hindex[k]]==levels(x[,Hindex[k]])[j]]),plot=FALSE)
					#remove zeros from counts
					q<-b$counts
					q[q==0]<-NA
					#transform counts to boxplot coordinates
					...
					#plot points from histogram
					if(length(b$mids)<2){
						points(y,unique(na.omit(x_all_cln0[[k]][,i][x[,Hindex[k]]==levels(x[,Hindex[k]])[j]])),col="blue")
					}
					else{
						points(y,b$mids,col="blue")
					}
					#plot mean as a line
					segments(j-0.4,mean(na.omit(x_all_cln0[[k]][,i][x[,Hindex[k]]==levels(x[,Hindex[k]])[j]])),j+0.4,mean(na.omit(x_all_cln0[[k]][,i][x[,Hindex[k]]==levels(x[,Hindex[k]])[j]])),col="red")
					#plot N for variable
					text(j,max(na.omit(x_all_cln0[[k]][,i][x[,Hindex[k]]==levels(x[,Hindex[k]])[j]])), labels=paste("N = ",length(na.omit(x_all_cln0[[k]][,i][x[,Hindex[k]]==levels(x[,Hindex[k]])[j]])),sep=""),cex=0.8,pos=3)
					#get range amplitude
					rango<-max(na.omit(x_all_cln0[[k]][,i]))-min(na.omit(x_all_cln0[[k]][,i]))
					#plot axis for histogram
					segments(j-0.4,min(na.omit(x_all_cln0[[k]][,i][x[,Hindex[k]]==levels(x[,Hindex[k]])[j]]))-(0.01*rango),j+0.4,min(na.omit(x_all_cln0[[k]][,i][x[,Hindex[k]]==levels(x[,Hindex[k]])[j]]))-(0.01*rango),col="grey",adj=0)
					#plot max line for histogram
					segments(j-0.4,min(na.omit(x_all_cln0[[k]][,i][x[,Hindex[k]]==levels(x[,Hindex[k]])[j]])),j-0.4,max(na.omit(x_all_cln0[[k]][,i][x[,Hindex[k]]==levels(x[,Hindex[k]])[j]])),lty=3,col="grey")
					segments(j-0.4,min(na.omit(x_all_cln0[[k]][,i][x[,Hindex[k]]==levels(x[,Hindex[k]])[j]]))-(0.02*rango),j-0.4,min(na.omit(x_all_cln0[[k]][,i][x[,Hindex[k]]==levels(x[,Hindex[k]])[j]])),col="grey")
					#plot mean line for histogram
					segments(j,min(na.omit(x_all_cln0[[k]][,i][x[,Hindex[k]]==levels(x[,Hindex[k]])[j]])),j,max(na.omit(x_all_cln0[[k]][,i][x[,Hindex[k]]==levels(x[,Hindex[k]])[j]])),lty=3,col="grey")
					segments(j,min(na.omit(x_all_cln0[[k]][,i][x[,Hindex[k]]==levels(x[,Hindex[k]])[j]]))-(0.02*rango),j,min(na.omit(x_all_cln0[[k]][,i][x[,Hindex[k]]==levels(x[,Hindex[k]])[j]])),col="grey")
					#plot min line for histogram
					segments(j+0.4,min(na.omit(x_all_cln0[[k]][,i][x[,Hindex[k]]==levels(x[,Hindex[k]])[j]])),j+0.4,max(na.omit(x_all_cln0[[k]][,i][x[,Hindex[k]]==levels(x[,Hindex[k]])[j]])),lty=3,col="grey")
					segments(j+0.4,min(na.omit(x_all_cln0[[k]][,i][x[,Hindex[k]]==levels(x[,Hindex[k]])[j]]))-(0.02*rango),j+0.4,min(na.omit(x_all_cln0[[k]][,i][x[,Hindex[k]]==levels(x[,Hindex[k]])[j]])),col="grey")
					#plot labels for the histogram axis
					text(j,min(na.omit(x_all_cln0[[k]][,i][x[,Hindex[k]]==levels(x[,Hindex[k]])[j]]))-(0.025*rango),labels="Count",cex=0.8)
					text(j-0.4,min(na.omit(x_all_cln0[[k]][,i][x[,Hindex[k]]==levels(x[,Hindex[k]])[j]]))-(0.03*rango),labels=max(na.omit(q)),cex=0.8)
					text(j+0.4,min(na.omit(x_all_cln0[[k]][,i][x[,Hindex[k]]==levels(x[,Hindex[k]])[j]]))-(0.03*rango),labels=min(na.omit(q)),cex=0.8)
				}	
			}
		dev.off()
	}
...
for(k in seq(Hlabel)){
	x_cln[[k]]<-x_cln0[[k]][!x_cln0[[k]]$ID%in%to_remove01,]
	x_all_cln[[k]]<-x_all_cln0[[k]][!x_cln0[[k]]$ID%in%to_remove01,]
	x_multi_cln[[k]]<-x_multi_cln0[[k]][!x_cln0[[k]]$ID%in%to_remove01,]
}

# write list of previouly removed terminals
removed_terminals<-x$ID[which(x$ID%in%to_remove01)]
...
for(k in seq(Hlabel)){
	#loop for each variable
	for(i in 1:ncol(x_all_cln[[k]])){
		outlier_values <- boxplot(x_all_cln[[k]][,i] ~ x_cln[[k]][,Hindex[k]],range=2,plot=F)$stats
		outliers_ind<-vector()
		for (j in seq(OTUs[[k]])) {
			outliers_ind<-c(outliers_ind,x_cln[[k]]$ID[x_cln[[k]][,Hindex[k]]==OTUs[[k]][j]][x_all_cln[[k]][x_cln[[k]][,Hindex[k]]==OTUs[[k]][j],i]<outlier_values[1,j]|x_all_cln[[k]][x_cln[[k]][,Hindex[k]]==OTUs[[k]][j],i]>outlier_values[5,j]])
		}
		out_check[[Hlabel[k]]][[colnames(x_all_cln[[k]])[i]]]<-unique(na.omit(outliers_ind))
		# names(out_check[k])[i]<-colnames(x_all_cln[[k]])[i]
	}
}

# write list of removed outliers
sink("./01_tables/Removed_Outliers.txt")
	print(out_check)
sink()
...
for(k in seq(Hlabel)){
	x_all_cln_noOut[[Hlabel[k]]]<-x_all_cln[[k]]
	for(i in seq(out_check[[k]])){
...
for(k in seq(Hlabel)){
	x_multi_cln_noOut[[Hlabel[k]]]<-x_multi_cln[[k]]
	for(i in seq(out_check[[k]])){
...
sink("./01_tables/Removed_Zero-Variance.txt")
	print(zero_var)
sink()
...
for(k in seq(Hlabel)){
	...
	for (i in 1:ncol(x_all_cln_noOut_noZ[[k]])){
		if(length(grep("cont",colnames(x_all_cln_noOut_noZ[[k]])[i]))==1|length(grep("rat_",colnames(x_all_cln_noOut_noZ[[k]])[i]))==1){
			if (fligner.test(x_all_cln_noOut_noZ[[k]][,i]~x_cln[[k]]$sex,data=x_all_cln_noOut_noZ[[k]])$p.value>=0.05) {
				dimorphism[[k]][i,1]<-"T.test"
				dimorphism[[k]][i,2]<-round(t.test(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="F",i],x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="M",i])$p.value,4)
			} else {
				dimorphism[[k]][i,1]<-"Wilcoxon.test"
				dimorphism[[k]][i,2]<-round(wilcox.test(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="F",i],x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="M",i])$p.value,4)
			}
		} else {
			dimorphism[[k]][i,1]<-"Wilcoxon.test"
			dimorphism[[k]][i,2]<-round(wilcox.test(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="F",i],x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="M",i])$p.value,4)
...
for(k in 1:length(Hlabel)){
	...
	for(i in 1:ncol(x_all_cln_noOut_noZ[[k]])){
		...
		pdf(paste("./04_boxplots/03_all_variables-",Hlabel[k],"/boxplot_",colnames(x_all_cln_noOut_noZ[[k]])[i],".pdf",sep=""),family="ArialMT", width=3+length(OTUs[[k]]), height=12)
			#plot boxplot
			par(mar = c(7, 4, 4, 2) + 0.1)
			boxplot(x_all_cln_noOut_noZ[[k]][,i] ~ x_cln[[k]][,Hindex[k]],main=colnames(x_all_cln_noOut_noZ[[k]])[i],outline=F,ylim=c(min(na.omit(x_all_cln_noOut_noZ[[k]][,i])),max(na.omit(x_all_cln_noOut_noZ[[k]][,i]))),boxwex = 0.8,xaxt = "n", xlab= Hlabel[k], ylab ="")
	# 		axis(1, labels = FALSE)
			xlabels <- levels(x_cln[[k]][,Hindex[k]])
			text(1:length(levels(x_cln[[k]][,Hindex[k]])), par("usr")[3], offset=1, srt = 45, adj = 1, labels = xlabels, xpd = TRUE)
			#loop for each OTUs
			for(j in 1:length(levels(x_cln[[k]][,Hindex[k]]))){
				if(length(na.omit(x_all_cln_noOut_noZ[[k]][,i][x_cln[[k]][,Hindex[k]]==levels(x_cln[[k]][,Hindex[k]])[j]]))==0){
					#plot N for variable
					text(j,mean(na.omit(x_all_cln_noOut_noZ[[k]][,i])), labels=paste("N = ",length(na.omit(x_all_cln_noOut_noZ[[k]][,i][x_cln[[k]][,Hindex[k]]==levels(x_cln[[k]][,Hindex[k]])[j]])),sep=""),cex=0.8,pos=3)
				}
				else{
					#get histogram data
					b<-hist(x_all_cln_noOut_noZ[[k]][,i][x_cln[[k]][,Hindex[k]]==levels(x_cln[[k]][,Hindex[k]])[j]],breaks=length(x_all_cln_noOut_noZ[[k]][,i][x_cln[[k]][,Hindex[k]]==levels(x_cln[[k]][,Hindex[k]])[j]]),plot=FALSE)
					#remove zeros from counts
					q<-b$counts
					q[q==0]<-NA
					#transform counts to boxplot coordinates
					...
					#plot points from histogram
					if(length(b$mids)<2){
						points(y,unique(na.omit(x_all_cln_noOut_noZ[[k]][,i][x_cln[[k]][,Hindex[k]]==levels(x_cln[[k]][,Hindex[k]])[j]])),col="blue")
					}
					else{
						points(y,b$mids,col="blue")
					}
					#plot mean as a line
					segments(j-0.4,mean(na.omit(x_all_cln_noOut_noZ[[k]][,i][x_cln[[k]][,Hindex[k]]==levels(x_cln[[k]][,Hindex[k]])[j]])),j+0.4,mean(na.omit(x_all_cln_noOut_noZ[[k]][,i][x_cln[[k]][,Hindex[k]]==levels(x_cln[[k]][,Hindex[k]])[j]])),col="red")
					#plot N for variable
					text(j,max(na.omit(x_all_cln_noOut_noZ[[k]][,i][x_cln[[k]][,Hindex[k]]==levels(x_cln[[k]][,Hindex[k]])[j]])), labels=paste("N = ",length(na.omit(x_all_cln_noOut_noZ[[k]][,i][x_cln[[k]][,Hindex[k]]==levels(x_cln[[k]][,Hindex[k]])[j]])),sep=""),cex=0.8,pos=3)
					#get range amplitude
					rango<-max(na.omit(x_all_cln_noOut_noZ[[k]][,i]))-min(na.omit(x_all_cln_noOut_noZ[[k]][,i]))
					#plot axis for histogram
					segments(j-0.4,min(na.omit(x_all_cln_noOut_noZ[[k]][,i][x_cln[[k]][,Hindex[k]]==levels(x_cln[[k]][,Hindex[k]])[j]]))-(0.01*rango),j+0.4,min(na.omit(x_all_cln_noOut_noZ[[k]][,i][x_cln[[k]][,Hindex[k]]==levels(x_cln[[k]][,Hindex[k]])[j]]))-(0.01*rango),col="grey",adj=0)
					#plot max line for histogram
					segments(j-0.4,min(na.omit(x_all_cln_noOut_noZ[[k]][,i][x_cln[[k]][,Hindex[k]]==levels(x_cln[[k]][,Hindex[k]])[j]])),j-0.4,max(na.omit(x_all_cln_noOut_noZ[[k]][,i][x_cln[[k]][,Hindex[k]]==levels(x_cln[[k]][,Hindex[k]])[j]])),lty=3,col="grey")
					segments(j-0.4,min(na.omit(x_all_cln_noOut_noZ[[k]][,i][x_cln[[k]][,Hindex[k]]==levels(x_cln[[k]][,Hindex[k]])[j]]))-(0.02*rango),j-0.4,min(na.omit(x_all_cln_noOut_noZ[[k]][,i][x_cln[[k]][,Hindex[k]]==levels(x_cln[[k]][,Hindex[k]])[j]])),col="grey")
					#plot mean line for histogram
					segments(j,min(na.omit(x_all_cln_noOut_noZ[[k]][,i][x_cln[[k]][,Hindex[k]]==levels(x_cln[[k]][,Hindex[k]])[j]])),j,max(na.omit(x_all_cln_noOut_noZ[[k]][,i][x_cln[[k]][,Hindex[k]]==levels(x_cln[[k]][,Hindex[k]])[j]])),lty=3,col="grey")
					segments(j,min(na.omit(x_all_cln_noOut_noZ[[k]][,i][x_cln[[k]][,Hindex[k]]==levels(x_cln[[k]][,Hindex[k]])[j]]))-(0.02*rango),j,min(na.omit(x_all_cln_noOut_noZ[[k]][,i][x_cln[[k]][,Hindex[k]]==levels(x_cln[[k]][,Hindex[k]])[j]])),col="grey")
					#plot min line for histogram
					segments(j+0.4,min(na.omit(x_all_cln_noOut_noZ[[k]][,i][x_cln[[k]][,Hindex[k]]==levels(x_cln[[k]][,Hindex[k]])[j]])),j+0.4,max(na.omit(x_all_cln_noOut_noZ[[k]][,i][x_cln[[k]][,Hindex[k]]==levels(x_cln[[k]][,Hindex[k]])[j]])),lty=3,col="grey")
					segments(j+0.4,min(na.omit(x_all_cln_noOut_noZ[[k]][,i][x_cln[[k]][,Hindex[k]]==levels(x_cln[[k]][,Hindex[k]])[j]]))-(0.02*rango),j+0.4,min(na.omit(x_all_cln_noOut_noZ[[k]][,i][x_cln[[k]][,Hindex[k]]==levels(x_cln[[k]][,Hindex[k]])[j]])),col="grey")
					#plot labels for the histogram axis
					text(j,min(na.omit(x_all_cln_noOut_noZ[[k]][,i][x_cln[[k]][,Hindex[k]]==levels(x_cln[[k]][,Hindex[k]])[j]]))-(0.025*rango),labels="Count",cex=0.8)
					text(j-0.4,min(na.omit(x_all_cln_noOut_noZ[[k]][,i][x_cln[[k]][,Hindex[k]]==levels(x_cln[[k]][,Hindex[k]])[j]]))-(0.03*rango),labels=max(na.omit(q)),cex=0.8)
					text(j+0.4,min(na.omit(x_all_cln_noOut_noZ[[k]][,i][x_cln[[k]][,Hindex[k]]==levels(x_cln[[k]][,Hindex[k]])[j]]))-(0.03*rango),labels=min(na.omit(q)),cex=0.8)
				}	
			}
		dev.off()
	}
	...
	for(i in 1:ncol(x_all_cln_noOut_noZ[[k]])){
		if(as.numeric(dimorphism[[k]][i,2])>=0.05){
			...
			pdf(paste("./04_boxplots/04_all_variables-",Hlabel[k],"-sex/boxplot_",colnames(x_all_cln_noOut_noZ[[k]])[i],".pdf",sep=""),family="ArialMT", width=10+length(OTUs[[k]]), height=14)
				...
				boxplot(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="F",i] ~ x_cln[[k]][,Hindex[k]][x_cln[[k]]$sex=="F"],main=colnames(x_all_cln_noOut_noZ[[k]])[i],outline=F,ylim=c(min(na.omit(x_all_cln_noOut_noZ[[k]][,i])),max(na.omit(x_all_cln_noOut_noZ[[k]][,i]))),boxwex = 0.8,xaxt = "n", xlab= paste(Hlabel[k]," Females",sep=""), ylab ="")
				xlabels <- levels(x_cln[[k]][,Hindex[k]])
				text(1:length(levels(x_cln[[k]][,Hindex[k]])), par("usr")[3], offset=1, srt = 45, adj = 1, labels = xlabels, xpd = TRUE)
				#loop for each OTUs
				for(j in 1:length(levels(x_cln[[k]][,Hindex[k]]))){
					if(length(na.omit(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="F",i][x_cln[[k]][,Hindex[k]][x_cln[[k]]$sex=="F"]==levels(x_cln[[k]][,Hindex[k]])[j]]))==0){
						#plot N for variable
						text(j,mean(na.omit(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="F",i])), labels=paste("N = ",length(na.omit(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="F",i][x_cln[[k]][,Hindex[k]][x_cln[[k]]$sex=="F"]==levels(x_cln[[k]][,Hindex[k]])[j]])),sep=""),cex=0.8,pos=3)
					}else{
						#get histogram data
						b<-hist(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="F",i][x_cln[[k]][,Hindex[k]][x_cln[[k]]$sex=="F"]==levels(x_cln[[k]][,Hindex[k]])[j]],breaks=length(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="F",i][x_cln[[k]][,Hindex[k]][x_cln[[k]]$sex=="F"]==levels(x_cln[[k]][,Hindex[k]])[j]]),plot=FALSE)
						#remove zeros from counts
						q<-b$counts
						q[q==0]<-NA
						#transform counts to boxplot coordinates
						...
						#plot points from histogram
						if(length(b$mids)<2){
							points(y,unique(na.omit(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="F",i][x_cln[[k]][,Hindex[k]][x_cln[[k]]$sex=="F"]==levels(x_cln[[k]][,Hindex[k]])[j]])),col="blue")
						}else{
							points(y,b$mids,col="blue")
						}
						#plot mean as a line
						segments(j-0.4,mean(na.omit(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="F",i][x_cln[[k]][,Hindex[k]][x_cln[[k]]$sex=="F"]==levels(x_cln[[k]][,Hindex[k]])[j]])),j+0.4,mean(na.omit(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="F",i][x_cln[[k]][,Hindex[k]][x_cln[[k]]$sex=="F"]==levels(x_cln[[k]][,Hindex[k]])[j]])),col="red")
						#plot N for variable
						text(j,max(na.omit(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="F",i][x_cln[[k]][,Hindex[k]][x_cln[[k]]$sex=="F"]==levels(x_cln[[k]][,Hindex[k]])[j]])), labels=paste("N = ",length(na.omit(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="F",i][x_cln[[k]][,Hindex[k]][x_cln[[k]]$sex=="F"]==levels(x_cln[[k]][,Hindex[k]])[j]])),sep=""),cex=0.8,pos=3)
						#get range amplitude
						rango<-max(na.omit(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="F",i]))-min(na.omit(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="F",i]))
						#plot axis for histogram
						segments(j-0.4,min(na.omit(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="F",i][x_cln[[k]][,Hindex[k]][x_cln[[k]]$sex=="F"]==levels(x_cln[[k]][,Hindex[k]])[j]]))-(0.01*rango),j+0.4,min(na.omit(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="F",i][x_cln[[k]][,Hindex[k]][x_cln[[k]]$sex=="F"]==levels(x_cln[[k]][,Hindex[k]])[j]]))-(0.01*rango),col="grey",adj=0)
						#plot max line for histogram
						segments(j-0.4,min(na.omit(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="F",i][x_cln[[k]][,Hindex[k]][x_cln[[k]]$sex=="F"]==levels(x_cln[[k]][,Hindex[k]])[j]])),j-0.4,max(na.omit(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="F",i][x_cln[[k]][,Hindex[k]][x_cln[[k]]$sex=="F"]==levels(x_cln[[k]][,Hindex[k]])[j]])),lty=3,col="grey")
						segments(j-0.4,min(na.omit(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="F",i][x_cln[[k]][,Hindex[k]][x_cln[[k]]$sex=="F"]==levels(x_cln[[k]][,Hindex[k]])[j]]))-(0.02*rango),j-0.4,min(na.omit(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="F",i][x_cln[[k]][,Hindex[k]][x_cln[[k]]$sex=="F"]==levels(x_cln[[k]][,Hindex[k]])[j]])),col="grey")
						#plot mean line for histogram
						segments(j,min(na.omit(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="F",i][x_cln[[k]][,Hindex[k]][x_cln[[k]]$sex=="F"]==levels(x_cln[[k]][,Hindex[k]])[j]])),j,max(na.omit(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="F",i][x_cln[[k]][,Hindex[k]][x_cln[[k]]$sex=="F"]==levels(x_cln[[k]][,Hindex[k]])[j]])),lty=3,col="grey")
						segments(j,min(na.omit(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="F",i][x_cln[[k]][,Hindex[k]][x_cln[[k]]$sex=="F"]==levels(x_cln[[k]][,Hindex[k]])[j]]))-(0.02*rango),j,min(na.omit(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="F",i][x_cln[[k]][,Hindex[k]][x_cln[[k]]$sex=="F"]==levels(x_cln[[k]][,Hindex[k]])[j]])),col="grey")
						#plot min line for histogram
						segments(j+0.4,min(na.omit(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="F",i][x_cln[[k]][,Hindex[k]][x_cln[[k]]$sex=="F"]==levels(x_cln[[k]][,Hindex[k]])[j]])),j+0.4,max(na.omit(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="F",i][x_cln[[k]][,Hindex[k]][x_cln[[k]]$sex=="F"]==levels(x_cln[[k]][,Hindex[k]])[j]])),lty=3,col="grey")
						segments(j+0.4,min(na.omit(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="F",i][x_cln[[k]][,Hindex[k]][x_cln[[k]]$sex=="F"]==levels(x_cln[[k]][,Hindex[k]])[j]]))-(0.02*rango),j+0.4,min(na.omit(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="F",i][x_cln[[k]][,Hindex[k]][x_cln[[k]]$sex=="F"]==levels(x_cln[[k]][,Hindex[k]])[j]])),col="grey")
						#plot labels for the histogram axis
						text(j,min(na.omit(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="F",i][x_cln[[k]][,Hindex[k]][x_cln[[k]]$sex=="F"]==levels(x_cln[[k]][,Hindex[k]])[j]]))-(0.025*rango),labels="Count",cex=0.8)
						text(j-0.4,min(na.omit(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="F",i][x_cln[[k]][,Hindex[k]][x_cln[[k]]$sex=="F"]==levels(x_cln[[k]][,Hindex[k]])[j]]))-(0.03*rango),labels=max(na.omit(q)),cex=0.8)
						text(j+0.4,min(na.omit(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="F",i][x_cln[[k]][,Hindex[k]][x_cln[[k]]$sex=="F"]==levels(x_cln[[k]][,Hindex[k]])[j]]))-(0.03*rango),labels=min(na.omit(q)),cex=0.8)
					}
				}
				#plot boxplot for males
				boxplot(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="M",i] ~ x_cln[[k]][,Hindex[k]][x_cln[[k]]$sex=="M"],main=colnames(x_all_cln_noOut_noZ[[k]])[i],outline=F,ylim=c(min(na.omit(x_all_cln_noOut_noZ[[k]][,i])),max(na.omit(x_all_cln_noOut_noZ[[k]][,i]))),boxwex = 0.8,xaxt = "n", xlab= paste(Hlabel[k]," Males",sep=""), ylab ="")
				xlabels <- levels(x_cln[[k]][,Hindex[k]])
				text(1:length(levels(x_cln[[k]][,Hindex[k]])), par("usr")[3], offset=1, srt = 45, adj = 1, labels = xlabels, xpd = TRUE)
				#loop for each OTUs
				for(j in 1:length(levels(x_cln[[k]][,Hindex[k]]))){
					if(length(na.omit(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="M",i][x_cln[[k]][,Hindex[k]][x_cln[[k]]$sex=="M"]==levels(x_cln[[k]][,Hindex[k]])[j]]))==0){
						#plot N for variable
						text(j,mean(na.omit(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="M",i])), labels=paste("N = ",length(na.omit(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="M",i][x_cln[[k]][,Hindex[k]][x_cln[[k]]$sex=="M"]==levels(x_cln[[k]][,Hindex[k]])[j]])),sep=""),cex=0.8,pos=3)
					}else{
						#get histogram data
						b<-hist(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="M",i][x_cln[[k]][,Hindex[k]][x_cln[[k]]$sex=="M"]==levels(x_cln[[k]][,Hindex[k]])[j]],breaks=length(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="M",i][x_cln[[k]][,Hindex[k]][x_cln[[k]]$sex=="M"]==levels(x_cln[[k]][,Hindex[k]])[j]]),plot=FALSE)
						#remove zeros from counts
						q<-b$counts
						q[q==0]<-NA
						#transform counts to boxplot coordinates
						...
						#plot points from histogram
						if(length(b$mids)<2){
							points(y,unique(na.omit(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="M",i][x_cln[[k]][,Hindex[k]][x_cln[[k]]$sex=="M"]==levels(x_cln[[k]][,Hindex[k]])[j]])),col="blue")
						}else{
							points(y,b$mids,col="blue")
						}
						#plot mean as a line
						segments(j-0.4,mean(na.omit(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="M",i][x_cln[[k]][,Hindex[k]][x_cln[[k]]$sex=="M"]==levels(x_cln[[k]][,Hindex[k]])[j]])),j+0.4,mean(na.omit(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="M",i][x_cln[[k]][,Hindex[k]][x_cln[[k]]$sex=="M"]==levels(x_cln[[k]][,Hindex[k]])[j]])),col="red")
						#plot N for variable
						text(j,max(na.omit(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="M",i][x_cln[[k]][,Hindex[k]][x_cln[[k]]$sex=="M"]==levels(x_cln[[k]][,Hindex[k]])[j]])), labels=paste("N = ",length(na.omit(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="M",i][x_cln[[k]][,Hindex[k]][x_cln[[k]]$sex=="M"]==levels(x_cln[[k]][,Hindex[k]])[j]])),sep=""),cex=0.8,pos=3)
						#get range amplitude
						rango<-max(na.omit(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="M",i]))-min(na.omit(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="M",i]))
						#plot axis for histogram
						segments(j-0.4,min(na.omit(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="M",i][x_cln[[k]][,Hindex[k]][x_cln[[k]]$sex=="M"]==levels(x_cln[[k]][,Hindex[k]])[j]]))-(0.01*rango),j+0.4,min(na.omit(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="M",i][x_cln[[k]][,Hindex[k]][x_cln[[k]]$sex=="M"]==levels(x_cln[[k]][,Hindex[k]])[j]]))-(0.01*rango),col="grey",adj=0)
						#plot max line for histogram
						segments(j-0.4,min(na.omit(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="M",i][x_cln[[k]][,Hindex[k]][x_cln[[k]]$sex=="M"]==levels(x_cln[[k]][,Hindex[k]])[j]])),j-0.4,max(na.omit(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="M",i][x_cln[[k]][,Hindex[k]][x_cln[[k]]$sex=="M"]==levels(x_cln[[k]][,Hindex[k]])[j]])),lty=3,col="grey")
						segments(j-0.4,min(na.omit(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="M",i][x_cln[[k]][,Hindex[k]][x_cln[[k]]$sex=="M"]==levels(x_cln[[k]][,Hindex[k]])[j]]))-(0.02*rango),j-0.4,min(na.omit(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="M",i][x_cln[[k]][,Hindex[k]][x_cln[[k]]$sex=="M"]==levels(x_cln[[k]][,Hindex[k]])[j]])),col="grey")
						#plot mean line for histogram
						segments(j,min(na.omit(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="M",i][x_cln[[k]][,Hindex[k]][x_cln[[k]]$sex=="M"]==levels(x_cln[[k]][,Hindex[k]])[j]])),j,max(na.omit(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="M",i][x_cln[[k]][,Hindex[k]][x_cln[[k]]$sex=="M"]==levels(x_cln[[k]][,Hindex[k]])[j]])),lty=3,col="grey")
						segments(j,min(na.omit(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="M",i][x_cln[[k]][,Hindex[k]][x_cln[[k]]$sex=="M"]==levels(x_cln[[k]][,Hindex[k]])[j]]))-(0.02*rango),j,min(na.omit(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="M",i][x_cln[[k]][,Hindex[k]][x_cln[[k]]$sex=="M"]==levels(x_cln[[k]][,Hindex[k]])[j]])),col="grey")
						#plot min line for histogram
						segments(j+0.4,min(na.omit(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="M",i][x_cln[[k]][,Hindex[k]][x_cln[[k]]$sex=="M"]==levels(x_cln[[k]][,Hindex[k]])[j]])),j+0.4,max(na.omit(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="M",i][x_cln[[k]][,Hindex[k]][x_cln[[k]]$sex=="M"]==levels(x_cln[[k]][,Hindex[k]])[j]])),lty=3,col="grey")
						segments(j+0.4,min(na.omit(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="M",i][x_cln[[k]][,Hindex[k]][x_cln[[k]]$sex=="M"]==levels(x_cln[[k]][,Hindex[k]])[j]]))-(0.02*rango),j+0.4,min(na.omit(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="M",i][x_cln[[k]][,Hindex[k]][x_cln[[k]]$sex=="M"]==levels(x_cln[[k]][,Hindex[k]])[j]])),col="grey")
						#plot labels for the histogram axis
						text(j,min(na.omit(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="M",i][x_cln[[k]][,Hindex[k]][x_cln[[k]]$sex=="M"]==levels(x_cln[[k]][,Hindex[k]])[j]]))-(0.025*rango),labels="Count",cex=0.8)
						text(j-0.4,min(na.omit(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="M",i][x_cln[[k]][,Hindex[k]][x_cln[[k]]$sex=="M"]==levels(x_cln[[k]][,Hindex[k]])[j]]))-(0.03*rango),labels=max(na.omit(q)),cex=0.8)
						text(j+0.4,min(na.omit(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="M",i][x_cln[[k]][,Hindex[k]][x_cln[[k]]$sex=="M"]==levels(x_cln[[k]][,Hindex[k]])[j]]))-(0.03*rango),labels=min(na.omit(q)),cex=0.8)
					}
				}
			dev.off()
		}
...
# correlogram for ALL MULTI (discrete, categorical and rates)
...
backup<-x_multi_cln_noOut_noZ_cln_noCor[[k]]

x_multi_cln_noOut_noZ_cln_noCor[[k]]<-x_multi_cln_noOut_noZ_cln_noCor[[k]][,-c(1,6)]
...
all_OTUs<-c("aff_melanocephala_1","boipiranga","cf_melanocephala","melanocephala", "aff_melanocephala_2")
# define the colors
myColors <- c("#01b47c","#de3163","#8b3a7b","#000080","#00e6e2")


# myColors <- gg_color_hue(length(all_OTUs))
...
Shape_numbers2<-list()
for(k in 1:length(Hindex)){
	Shape_numbers1[[Hlabel[k]]]<-as.character(x_cln_noOut[[k]]$Types)
	Shape_numbers2[[Hlabel[k]]]<-as.character(x_cln_noOut[[k]]$Types)
	Shape_numbers2[[Hlabel[k]]][Shape_numbers2[[Hlabel[k]]]=="synonym"]<-"synonym2"
	Shape_numbers2[[Hlabel[k]]][Shape_numbers2[[Hlabel[k]]]=="regular"]<-NA
	Shape_numbers2[[Hlabel[k]]][Shape_numbers2[[Hlabel[k]]]=="type"]<-NA
}

myShapes<-c('synonym'=24, 'synonym2'=25, 'regular'=16,  'type'=8)
...
# plot PCA for RATES, CATEGORICAL and DISC variables - polygon instead of ellipse 
for(k in seq(Hlabel)){
	...
	forChull<-cbind(dfscores[,1:2],OTUS=x_cln_noOut[[k]][,Hindex[k]],Labels=x_cln_noOut[[k]]$ID)
	...
	g <- g + geom_point(data=forChull, aes(x=`PC1`,y=`PC2`, color = OTUS, fill=OTUS), size=1.5)
	# g <- g + geom_text(data=forChull, aes(x=`PC1`,y=`PC2`, color = OTUS, label=Labels), hjust = -0.5, nudge_x = 0.0) # uncomment this line if you want to check the lables associated to each point (IDs)
	...
	g1<-g
	# pdf(paste("./06_PCAs/01_PCA_multi_variables-",Hlabel[k],"/PCA-plot3.pdf",sep=""), family="ArialMT", width=8, height=11)
	# 	print(g1)
	# dev.off()	

	# plot pca for females - ALL variables
	pca_multi_F <- prcomp(x_multi_cln_noOut_noZ_cln_noCor[[k]][x_cln[[k]]$sex=="F",], scale.=F)
	dfscores_F <- as.data.frame(pca_multi_F$x)
	prop_pca_F = pca_multi_F$sdev^2/sum(pca_multi_F$sdev^2)
	forChull<-cbind(dfscores_F[,1:2],OTUS=x_cln_noOut[[k]][,Hindex[k]][x_cln_noOut[[k]]$sex=="F"],Labels=x_cln_noOut[[k]]$ID[x_cln_noOut[[k]]$sex=="F"])
	forChull$OTUS<-factor(forChull$OTUS,all_OTUs[all_OTUs%in%forChull$OTUS])
	afterChull<-vector()
	for(i in 1:length(OTUs[[k]])){
		afterChull<-rbind(afterChull,forChull[forChull[,3]==OTUs[[k]][i],][chull(forChull[forChull[,3]==OTUs[[k]][i],1:2]),])
	...
	g2<-g
	# pdf(paste("./06_PCAs/01_PCA_multi_variables-",Hlabel[k],"/PCA-plot-F2.pdf",sep=""), family="ArialMT", width=8, height=11)
	# 	print(g2)
	# dev.off()
	
	# plot pca for males - ALL variables
	pca_multi_M <- prcomp(x_multi_cln_noOut_noZ_cln_noCor[[k]][x_cln[[k]]$sex=="M",], scale.=F)
	dfscores_M <- as.data.frame(pca_multi_M$x)
	prop_pca_M = pca_multi_M$sdev^2/sum(pca_multi_M$sdev^2)
	forChull<-cbind(dfscores_M[,1:2],OTUS=x_cln_noOut[[k]][,Hindex[k]][x_cln_noOut[[k]]$sex=="M"],Labels=x_cln_noOut[[k]]$ID[x_cln_noOut[[k]]$sex=="M"])
	forChull$OTUS<-factor(forChull$OTUS,all_OTUs[all_OTUs%in%forChull$OTUS])
	afterChull<-vector()
	for(i in 1:length(OTUs[[k]])){
		afterChull<-rbind(afterChull,forChull[forChull[,3]==OTUs[[k]][i],][chull(forChull[forChull[,3]==OTUs[[k]][i],1:2]),])
	...
	g3<-g
	# pdf(paste("./06_PCAs/01_PCA_multi_variables-",Hlabel[k],"/PCA-plot-M2.pdf",sep=""), family="ArialMT", width=8, height=11)
	# 	print(g3)
	# dev.off()

	# plot all combined
	pdf(paste("./06_PCAs/01_PCA_multi_variables-",Hlabel[k],"/PCA-plot-combined.pdf",sep=""), family="ArialMT", width=8, height=11)
...
for(k in seq(Hlabel)){
	...
	g <- g + geom_point(data=forChull, aes(x=`Comp 1`,y=`Comp 2`, color = OTUS, fill=OTUS), size=3)
...
sink(paste("./01_tables/cmPls_F-",Hlabel[k],"-Statistics.txt",sep=""))
	print(cmPls_F)
sink()

sink(paste("./01_tables/cmPls_M-",Hlabel[k],"-Statistics.txt",sep=""))
	print(cmPls_M)
sink()

sink(paste("./01_tables/cmPls-",Hlabel[k],"-Statistics.txt",sep=""))
	print(cmPls)
sink()
...
for(k in seq(Hlabel)){
	dir.create(paste("./09_Combined/01_combined-",Hlabel[k],sep=""),showWarnings = FALSE)
		...
		# plot pca for females - ALL variables
		pca_multi_F <- prcomp(x_multi_cln_noOut_noZ_cln_noCor[[k]][x_cln_noOut[[k]]$sex=="F",], scale.=F)
		dfscores_F <- as.data.frame(pca_multi_F$x)
		prop_pca_F = pca_multi_F$sdev^2/sum(pca_multi_F$sdev^2)
		...
		g <- g + guides(shape="none")
		g <- g + labs(x = paste("Comp 1 (", prop_var_F[1], "%)", sep=""), y = paste("Comp 2 (", prop_var_F[2], "%)", sep=""))
		g_PSLDA_F[[Hlabel[k]]]<-g

		# plot pca for males - ALL variables
		pca_multi_M <- prcomp(x_multi_cln_noOut_noZ_cln_noCor[[k]][x_cln_noOut[[k]]$sex=="M",], scale.=T)
		dfscores_M <- as.data.frame(pca_multi_M$x)
		prop_pca_M = pca_multi_M$sdev^2/sum(pca_multi_M$sdev^2)
		...
		g <- g + guides(shape="none")
		g <- g + labs(x = paste("Comp 1 (", prop_var_M[1], "%)", sep=""), y = paste("Comp 2 (", prop_var_M[2], "%)", sep=""))
		g_PSLDA_M[[Hlabel[k]]]<-g

		# plot pca for ALL - ALL variables
		pca_multi <- prcomp(x_multi_cln_noOut_noZ_cln_noCor[[k]], scale.=T)
		dfscores <- as.data.frame(pca_multi$x)
		prop_pca = pca_multi$sdev^2/sum(pca_multi$sdev^2)
		...
		control_train<- trainControl("repeatedcv", index = myfolds_train[[k]], selectionFunction = "oneSE")
		plsda_multi[[Hlabel[k]]]<-train(x=x_multi_cln_noOut_noZ_cln_noCor[[k]],y=x_cln_noOut[[k]][,Hindex[k]],method="pls",tuneLength=30,trControl=control,preProc=c("zv","center","scale"),metric="Accuracy", maxit = 10000, verbose=T)
		dfscores <- as.data.frame(plsda_multi[[k]]$finalModel$scores[,])
		prop_var <- round(plsda_multi[[k]]$finalModel$Xvar/sum(plsda_multi[[k]]$finalModel$Xvar)*100, 1)
		forChull<-cbind(dfscores[,1:2],OTUS=x_cln_noOut[[k]][,Hindex[k]],Labels=x_cln_noOut[[k]]$ID,Shapes1=Shape_numbers1[[Hlabel[k]]],Shapes2=Shape_numbers2[[Hlabel[k]]])
		forChull$OTUS<-factor(forChull$OTUS,all_OTUs[all_OTUs%in%forChull$OTUS])
		afterChull<-vector()
		for(i in 1:length(OTUs[[k]])){
...
pdf("./09_Combined/01_combined-F.pdf", family="ArialMT", width=8.3, height=10.7)

	grid.arrange(g_PCA_F[[1]], g_PSLDA_F[[1]], g_PCA_F[[2]], g_PSLDA_F[[2]], g_PCA_F[[3]], g_PSLDA_F[[3]], nrow = 3, ncol = 2)

dev.off()

pdf("./09_Combined/01_combined-M.pdf", family="ArialMT", width=8.3, height=10.7)

	grid.arrange(g_PCA_M[[1]], g_PSLDA_M[[1]], g_PCA_M[[2]], g_PSLDA_M[[2]], g_PCA_M[[3]], g_PSLDA_M[[3]], nrow = 3, ncol = 2)

dev.off()

pdf("./09_Combined/01_combined.pdf", family="ArialMT", width=8.3, height=10.7)

	grid.arrange(g_PCA[[1]], g_PSLDA[[1]], g_PCA[[2]], g_PSLDA[[2]], g_PCA[[3]], g_PSLDA[[3]], nrow = 3, ncol = 2)
...
for(k in 1:length(Hlabel)){
	...
	#open pdf file
	pdf(paste("./09_Combined/01_combined-",Hlabel[k],"-sex/boxplot.pdf",sep=""),family="ArialMT", width=6, height=9)
		layout(matrix(c(seq(length(vip)*2)),length(vip),2,byrow=T))
		par(mar=c(4,2,2,2))
		#loop for each variable
		for(i in seq(vipInd)){
			#plot boxplot for females
			boxplot(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="F",vipInd[i]] ~ x_cln[[k]][,Hindex[k]][x_cln[[k]]$sex=="F"],main=paste(gsub("disc_","",colnames(x_all_cln_noOut_noZ[[k]])[vipInd[i]]),"- Female",sep=" "), outline=F, ylim=c(min(na.omit(x_all_cln_noOut_noZ[[k]][,vipInd[i]]))-(diff(range(na.omit(x_all_cln_noOut_noZ[[k]][,vipInd[i]])))*0.02),max(na.omit(x_all_cln_noOut_noZ[[k]][,vipInd[i]]))+(diff(range(na.omit(x_all_cln_noOut_noZ[[k]][,vipInd[i]])))*0.02)),boxwex = 0.8,xaxt = "n", xlab= "", ylab ="",col=myColors[levels(x_cln[[k]][,Hindex[k]])%in%names(myColors)])
			xlabels <- levels(x_cln[[k]][,Hindex[k]])
			text(1:length(levels(x_cln[[k]][,Hindex[k]])), par("usr")[3], offset=1, srt = 45, adj = 1, labels = xlabels, xpd = TRUE, cex=0.8)
			#loop for each OTUs
			for(j in 1:length(levels(x_cln[[k]][,Hindex[k]]))){
				if(length(na.omit(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="F",vipInd[i]][x_cln[[k]][,Hindex[k]][x_cln[[k]]$sex=="F"]==levels(x_cln[[k]][,Hindex[k]])[j]]))==0){
					#plot N for variable
					text(j,mean(na.omit(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="F",vipInd[i]])), labels=paste("N = ",length(na.omit(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="F",vipInd[i]][x_cln[[k]][,Hindex[k]][x_cln[[k]]$sex=="F"]==levels(x_cln[[k]][,Hindex[k]])[j]])),sep=""),cex=0.8,pos=3)
				}else{
					#get histogram data
					# b<-hist(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="F",vipInd[i]][x_cln[[k]][,Hindex[k]][x_cln[[k]]$sex=="F"]==levels(x_cln[[k]][,Hindex[k]])[j]],breaks=length(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="F",vipInd[i]][x_cln[[k]][,Hindex[k]][x_cln[[k]]$sex=="F"]==levels(x_cln[[k]][,Hindex[k]])[j]]),plot=FALSE)
					
					#remove zeros from counts
					# q<-b$counts
					q<-as.numeric(table(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="F",vipInd[i]][x_cln[[k]][,Hindex[k]][x_cln[[k]]$sex=="F"]==levels(x_cln[[k]][,Hindex[k]])[j]]))
					# q[q==0]<-NA
					#transform counts to boxplot coordinates
					m<-((j-0.4)-(j+0.4))/(max(na.omit(q))-min(na.omit(q)))
					if(m!=-Inf){
					...
					#plot points from histogram
					if(length(q)<2){
						points(y,unique(na.omit(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="F",vipInd[i]][x_cln[[k]][,Hindex[k]][x_cln[[k]]$sex=="F"]==levels(x_cln[[k]][,Hindex[k]])[j]])),col="black",cex=0.7)
					}else{
						# points(y,b$mids,col="blue")
						points(y,as.numeric(attr(table(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="F",vipInd[i]][x_cln[[k]][,Hindex[k]][x_cln[[k]]$sex=="F"]==levels(x_cln[[k]][,Hindex[k]])[j]]),"dimnames")[[1]]),col="black",cex=0.7)
					}
					#plot mean as a line
					segments(j-0.4,mean(na.omit(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="F",vipInd[i]][x_cln[[k]][,Hindex[k]][x_cln[[k]]$sex=="F"]==levels(x_cln[[k]][,Hindex[k]])[j]])),j+0.4,mean(na.omit(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="F",vipInd[i]][x_cln[[k]][,Hindex[k]][x_cln[[k]]$sex=="F"]==levels(x_cln[[k]][,Hindex[k]])[j]])),col="red")
					#plot N for variable
					text(j,max(na.omit(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="F",vipInd[i]][x_cln[[k]][,Hindex[k]][x_cln[[k]]$sex=="F"]==levels(x_cln[[k]][,Hindex[k]])[j]])), labels=paste("N = ",length(na.omit(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="F",vipInd[i]][x_cln[[k]][,Hindex[k]][x_cln[[k]]$sex=="F"]==levels(x_cln[[k]][,Hindex[k]])[j]])),sep=""),cex=0.8,pos=3)
					#get range amplitude
					rango<-max(na.omit(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="F",vipInd[i]]))-min(na.omit(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="F",vipInd[i]]))
					#plot axis for histogram
					segments(j-0.4,min(na.omit(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="F",vipInd[i]][x_cln[[k]][,Hindex[k]][x_cln[[k]]$sex=="F"]==levels(x_cln[[k]][,Hindex[k]])[j]]))-(0.01*rango),j+0.4,min(na.omit(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="F",vipInd[i]][x_cln[[k]][,Hindex[k]][x_cln[[k]]$sex=="F"]==levels(x_cln[[k]][,Hindex[k]])[j]]))-(0.01*rango),col="grey",adj=0)
					#plot max line for histogram
					segments(j-0.4,min(na.omit(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="F",vipInd[i]][x_cln[[k]][,Hindex[k]][x_cln[[k]]$sex=="F"]==levels(x_cln[[k]][,Hindex[k]])[j]])),j-0.4,max(na.omit(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="F",vipInd[i]][x_cln[[k]][,Hindex[k]][x_cln[[k]]$sex=="F"]==levels(x_cln[[k]][,Hindex[k]])[j]])),lty=3,col="grey")
					segments(j-0.4,min(na.omit(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="F",vipInd[i]][x_cln[[k]][,Hindex[k]][x_cln[[k]]$sex=="F"]==levels(x_cln[[k]][,Hindex[k]])[j]]))-(0.02*rango),j-0.4,min(na.omit(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="F",vipInd[i]][x_cln[[k]][,Hindex[k]][x_cln[[k]]$sex=="F"]==levels(x_cln[[k]][,Hindex[k]])[j]])),col="grey")
					#plot mean line for histogram
					segments(j,min(na.omit(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="F",vipInd[i]][x_cln[[k]][,Hindex[k]][x_cln[[k]]$sex=="F"]==levels(x_cln[[k]][,Hindex[k]])[j]])),j,max(na.omit(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="F",vipInd[i]][x_cln[[k]][,Hindex[k]][x_cln[[k]]$sex=="F"]==levels(x_cln[[k]][,Hindex[k]])[j]])),lty=3,col="grey")
					segments(j,min(na.omit(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="F",vipInd[i]][x_cln[[k]][,Hindex[k]][x_cln[[k]]$sex=="F"]==levels(x_cln[[k]][,Hindex[k]])[j]]))-(0.02*rango),j,min(na.omit(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="F",vipInd[i]][x_cln[[k]][,Hindex[k]][x_cln[[k]]$sex=="F"]==levels(x_cln[[k]][,Hindex[k]])[j]])),col="grey")
					#plot min line for histogram
					segments(j+0.4,min(na.omit(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="F",vipInd[i]][x_cln[[k]][,Hindex[k]][x_cln[[k]]$sex=="F"]==levels(x_cln[[k]][,Hindex[k]])[j]])),j+0.4,max(na.omit(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="F",vipInd[i]][x_cln[[k]][,Hindex[k]][x_cln[[k]]$sex=="F"]==levels(x_cln[[k]][,Hindex[k]])[j]])),lty=3,col="grey")
					segments(j+0.4,min(na.omit(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="F",vipInd[i]][x_cln[[k]][,Hindex[k]][x_cln[[k]]$sex=="F"]==levels(x_cln[[k]][,Hindex[k]])[j]]))-(0.02*rango),j+0.4,min(na.omit(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="F",vipInd[i]][x_cln[[k]][,Hindex[k]][x_cln[[k]]$sex=="F"]==levels(x_cln[[k]][,Hindex[k]])[j]])),col="grey")
					#plot labels for the histogram axis
					text(j,min(na.omit(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="F",vipInd[i]][x_cln[[k]][,Hindex[k]][x_cln[[k]]$sex=="F"]==levels(x_cln[[k]][,Hindex[k]])[j]]))-(0.025*rango),labels="Count",cex=0.8)
					text(j-0.4,min(na.omit(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="F",vipInd[i]][x_cln[[k]][,Hindex[k]][x_cln[[k]]$sex=="F"]==levels(x_cln[[k]][,Hindex[k]])[j]]))-(0.03*rango),labels=max(na.omit(q)),cex=0.8)
					text(j+0.4,min(na.omit(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="F",vipInd[i]][x_cln[[k]][,Hindex[k]][x_cln[[k]]$sex=="F"]==levels(x_cln[[k]][,Hindex[k]])[j]]))-(0.03*rango),labels=min(na.omit(q)),cex=0.8)
				}
			}
			#plot boxplot for males
			boxplot(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="M",vipInd[i]] ~ x_cln[[k]][,Hindex[k]][x_cln[[k]]$sex=="M"],main=paste(gsub("disc_","",colnames(x_all_cln_noOut_noZ[[k]])[vipInd[i]]),"- Male",sep=" "),outline=F,ylim=c(min(na.omit(x_all_cln_noOut_noZ[[k]][,vipInd[i]]))-(diff(range(na.omit(x_all_cln_noOut_noZ[[k]][,vipInd[i]])))*0.02),max(na.omit(x_all_cln_noOut_noZ[[k]][,vipInd[i]]))+(diff(range(na.omit(x_all_cln_noOut_noZ[[k]][,vipInd[i]])))*0.02)),boxwex = 0.8,xaxt = "n", xlab= "", ylab ="",col=myColors[levels(x_cln[[k]][,Hindex[k]])%in%names(myColors)])
			xlabels <- levels(x_cln[[k]][,Hindex[k]])
			text(1:length(levels(x_cln[[k]][,Hindex[k]])), par("usr")[3], offset=1, srt = 45, adj = 1, labels = xlabels, xpd = TRUE, cex=0.8)
			#loop for each OTUs
			for(j in 1:length(levels(x_cln[[k]][,Hindex[k]]))){
				if(length(na.omit(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="M",vipInd[i]][x_cln[[k]][,Hindex[k]][x_cln[[k]]$sex=="M"]==levels(x_cln[[k]][,Hindex[k]])[j]]))==0){
					#plot N for variable
					text(j,mean(na.omit(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="M",vipInd[i]])), labels=paste("N = ",length(na.omit(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="M",vipInd[i]][x_cln[[k]][,Hindex[k]][x_cln[[k]]$sex=="M"]==levels(x_cln[[k]][,Hindex[k]])[j]])),sep=""),cex=0.8,pos=3)
				}else{
					#get histogram data
					# b<-hist(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="M",vipInd[i]][x_cln[[k]][,Hindex[k]][x_cln[[k]]$sex=="M"]==levels(x_cln[[k]][,Hindex[k]])[j]],breaks=length(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="M",vipInd[i]][x_cln[[k]][,Hindex[k]][x_cln[[k]]$sex=="M"]==levels(x_cln[[k]][,Hindex[k]])[j]]),plot=FALSE)
					#remove zeros from counts
					# q<-b$counts
					# q[q==0]<-NA
					q<-as.numeric(table(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="M",vipInd[i]][x_cln[[k]][,Hindex[k]][x_cln[[k]]$sex=="M"]==levels(x_cln[[k]][,Hindex[k]])[j]]))
					#transform counts to boxplot coordinates
					m<-((j-0.4)-(j+0.4))/(max(na.omit(q))-min(na.omit(q)))
					if(m!=-Inf){
						y<-(m*q)+((j+0.4)-(m*min(na.omit(q))))
					...
					#plot points from histogram
					if(length(q)<2){
						points(y,unique(na.omit(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="M",vipInd[i]][x_cln[[k]][,Hindex[k]][x_cln[[k]]$sex=="M"]==levels(x_cln[[k]][,Hindex[k]])[j]])),col="black",cex=0.7)
					}else{
						# points(y,b$mids,col="blue")
						points(y,as.numeric(attr(table(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="M",vipInd[i]][x_cln[[k]][,Hindex[k]][x_cln[[k]]$sex=="M"]==levels(x_cln[[k]][,Hindex[k]])[j]]),"dimnames")[[1]]),col="black",cex=0.7)
					}
					#plot mean as a line
					segments(j-0.4,mean(na.omit(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="M",vipInd[i]][x_cln[[k]][,Hindex[k]][x_cln[[k]]$sex=="M"]==levels(x_cln[[k]][,Hindex[k]])[j]])),j+0.4,mean(na.omit(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="M",vipInd[i]][x_cln[[k]][,Hindex[k]][x_cln[[k]]$sex=="M"]==levels(x_cln[[k]][,Hindex[k]])[j]])),col="red")
					#plot N for variable
					text(j,max(na.omit(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="M",vipInd[i]][x_cln[[k]][,Hindex[k]][x_cln[[k]]$sex=="M"]==levels(x_cln[[k]][,Hindex[k]])[j]])), labels=paste("N = ",length(na.omit(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="M",vipInd[i]][x_cln[[k]][,Hindex[k]][x_cln[[k]]$sex=="M"]==levels(x_cln[[k]][,Hindex[k]])[j]])),sep=""),cex=0.8,pos=3)
					#get range amplitude
					rango<-max(na.omit(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="M",vipInd[i]]))-min(na.omit(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="M",vipInd[i]]))
					#plot axis for histogram
					segments(j-0.4,min(na.omit(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="M",vipInd[i]][x_cln[[k]][,Hindex[k]][x_cln[[k]]$sex=="M"]==levels(x_cln[[k]][,Hindex[k]])[j]]))-(0.01*rango),j+0.4,min(na.omit(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="M",vipInd[i]][x_cln[[k]][,Hindex[k]][x_cln[[k]]$sex=="M"]==levels(x_cln[[k]][,Hindex[k]])[j]]))-(0.01*rango),col="grey",adj=0)
					#plot max line for histogram
					segments(j-0.4,min(na.omit(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="M",vipInd[i]][x_cln[[k]][,Hindex[k]][x_cln[[k]]$sex=="M"]==levels(x_cln[[k]][,Hindex[k]])[j]])),j-0.4,max(na.omit(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="M",vipInd[i]][x_cln[[k]][,Hindex[k]][x_cln[[k]]$sex=="M"]==levels(x_cln[[k]][,Hindex[k]])[j]])),lty=3,col="grey")
					segments(j-0.4,min(na.omit(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="M",vipInd[i]][x_cln[[k]][,Hindex[k]][x_cln[[k]]$sex=="M"]==levels(x_cln[[k]][,Hindex[k]])[j]]))-(0.02*rango),j-0.4,min(na.omit(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="M",vipInd[i]][x_cln[[k]][,Hindex[k]][x_cln[[k]]$sex=="M"]==levels(x_cln[[k]][,Hindex[k]])[j]])),col="grey")
					#plot mean line for histogram
					segments(j,min(na.omit(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="M",vipInd[i]][x_cln[[k]][,Hindex[k]][x_cln[[k]]$sex=="M"]==levels(x_cln[[k]][,Hindex[k]])[j]])),j,max(na.omit(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="M",vipInd[i]][x_cln[[k]][,Hindex[k]][x_cln[[k]]$sex=="M"]==levels(x_cln[[k]][,Hindex[k]])[j]])),lty=3,col="grey")
					segments(j,min(na.omit(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="M",vipInd[i]][x_cln[[k]][,Hindex[k]][x_cln[[k]]$sex=="M"]==levels(x_cln[[k]][,Hindex[k]])[j]]))-(0.02*rango),j,min(na.omit(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="M",vipInd[i]][x_cln[[k]][,Hindex[k]][x_cln[[k]]$sex=="M"]==levels(x_cln[[k]][,Hindex[k]])[j]])),col="grey")
					#plot min line for histogram
					segments(j+0.4,min(na.omit(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="M",vipInd[i]][x_cln[[k]][,Hindex[k]][x_cln[[k]]$sex=="M"]==levels(x_cln[[k]][,Hindex[k]])[j]])),j+0.4,max(na.omit(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="M",vipInd[i]][x_cln[[k]][,Hindex[k]][x_cln[[k]]$sex=="M"]==levels(x_cln[[k]][,Hindex[k]])[j]])),lty=3,col="grey")
					segments(j+0.4,min(na.omit(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="M",vipInd[i]][x_cln[[k]][,Hindex[k]][x_cln[[k]]$sex=="M"]==levels(x_cln[[k]][,Hindex[k]])[j]]))-(0.02*rango),j+0.4,min(na.omit(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="M",vipInd[i]][x_cln[[k]][,Hindex[k]][x_cln[[k]]$sex=="M"]==levels(x_cln[[k]][,Hindex[k]])[j]])),col="grey")
					#plot labels for the histogram axis
					text(j,min(na.omit(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="M",vipInd[i]][x_cln[[k]][,Hindex[k]][x_cln[[k]]$sex=="M"]==levels(x_cln[[k]][,Hindex[k]])[j]]))-(0.025*rango),labels="Count",cex=0.8)
					text(j-0.4,min(na.omit(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="M",vipInd[i]][x_cln[[k]][,Hindex[k]][x_cln[[k]]$sex=="M"]==levels(x_cln[[k]][,Hindex[k]])[j]]))-(0.03*rango),labels=max(na.omit(q)),cex=0.8)
					text(j+0.4,min(na.omit(x_all_cln_noOut_noZ[[k]][x_cln[[k]]$sex=="M",vipInd[i]][x_cln[[k]][,Hindex[k]][x_cln[[k]]$sex=="M"]==levels(x_cln[[k]][,Hindex[k]])[j]]))-(0.03*rango),labels=min(na.omit(q)),cex=0.8)
				}
			}
		}
	dev.off()
...
for(k in seq(Hlabel)){
	# PerMANOVA on RATES and DISC variables (Multi)
	...
	write.table(out_permanova_multi[[Hlabel[k]]]$aov.tab,paste("01_tables/PerMANOVA_Multi-",Hlabel[k],".txt",sep=""),quote=F, sep="\t", col.names=NA)
	...
	#######################################
	# PerMANOVA on RATES and DISC variables (Multi) - sex
...
for(k in 1:length(Hlabel)){
	...
	for (i in 1:length(OTUs)){
		all_up<-character()
		all_down<-character()
		for(j in 1:length(x_multi_cln[[k]][1,])){
			aA<-round(mean(na.omit(x_multi_cln[[k]][x_cln[[k]][,Hindex[k]]==OTUs[i],j])),2)
			bA<-round(sd(na.omit(x_multi_cln[[k]][x_cln[[k]][,Hindex[k]]==OTUs[i],j])),2)
			cA<-round(min(na.omit(x_multi_cln[[k]][x_cln[[k]][,Hindex[k]]==OTUs[i],j])),2)
			dA<-round(max(na.omit(x_multi_cln[[k]][x_cln[[k]][,Hindex[k]]==OTUs[i],j])),2)
			eA<-length(na.omit(x_multi_cln[[k]][x_cln[[k]][,Hindex[k]]==OTUs[i],j]))
			mA<-round(median(na.omit(x_multi_cln[[k]][x_cln[[k]][,Hindex[k]]==OTUs[i],j])),2)
	...
	rownames(data_summary_up)<-colnames(x_multi_cln[[k]])
...
for(k in 1:length(Hlabel)){
	...
	for (i in 1:length(OTUs)){
		...
		for(j in 1:length(x_multi_cln[[k]][1,])){
			aF<-round(mean(na.omit(x_multi_cln[[k]][x_cln[[k]][,Hindex[k]]==OTUs[i]&x_cln[[k]]$sex=="F",j])),2)
			bF<-round(sd(na.omit(x_multi_cln[[k]][x_cln[[k]][,Hindex[k]]==OTUs[i]&x_cln[[k]]$sex=="F",j])),2)
			cF<-round(min(na.omit(x_multi_cln[[k]][x_cln[[k]][,Hindex[k]]==OTUs[i]&x_cln[[k]]$sex=="F",j])),2)
			dF<-round(max(na.omit(x_multi_cln[[k]][x_cln[[k]][,Hindex[k]]==OTUs[i]&x_cln[[k]]$sex=="F",j])),2)
			eF<-length(na.omit(x_multi_cln[[k]][x_cln[[k]][,Hindex[k]]==OTUs[i]&x_cln[[k]]$sex=="F",j]))
			mF<-round(median(na.omit(x_multi_cln[[k]][x_cln[[k]][,Hindex[k]]==OTUs[i]&x_cln[[k]]$sex=="F",j])),2)
			fF<-(paste("range=",cF,"-",dF,", n=",eF,sep=""))
			gF<-c("x=",aF,", s=",bF,", m=",mF)
			hF<-paste(gF,collapse ="")
			females_up<-c(females_up,fF)
			females_down<-c(females_down,hF)
			aM<-round(mean(na.omit(x_multi_cln[[k]][x_cln[[k]][,Hindex[k]]==OTUs[i]&x_cln[[k]]$sex=="M",j])),2)
			bM<-round(sd(na.omit(x_multi_cln[[k]][x_cln[[k]][,Hindex[k]]==OTUs[i]&x_cln[[k]]$sex=="M",j])),2)
			cM<-round(min(na.omit(x_multi_cln[[k]][x_cln[[k]][,Hindex[k]]==OTUs[i]&x_cln[[k]]$sex=="M",j])),2)
			dM<-round(max(na.omit(x_multi_cln[[k]][x_cln[[k]][,Hindex[k]]==OTUs[i]&x_cln[[k]]$sex=="M",j])),2)
			eM<-length(na.omit(x_multi_cln[[k]][x_cln[[k]][,Hindex[k]]==OTUs[i]&x_cln[[k]]$sex=="M",j]))
			mM<-round(median(na.omit(x_multi_cln[[k]][x_cln[[k]][,Hindex[k]]==OTUs[i]&x_cln[[k]]$sex=="M",j])),2)
	...
	rownames(data_summary_up)<-gsub("cont_","",rownames(data_summary_up))
	rownames(data_summary_up)<-gsub("rat_","",rownames(data_summary_up))
	rownames(data_summary_down)<-rep("",length(x_multi_cln[[k]][1,]))
